...
let { entries, tags: tagList }: { entries: any[]; tags: string[] } = $props();

/** Track initial load to parse URL parameters */
let initial = $state(true);

/** Pagination size */
const size: number = config.pagination?.notes || 24;
...
let list: any[] = $derived.by(() => {
	let filtered: any[] = entries
		...
		.filter(entry => tags.every(tag => entry.data.tags?.includes(tag)))
...
<main class="flex flex-col-reverse sm:flex-row gap-10 grow">
	<article class="flex flex-col grow">
		<header class="grid grid-cols-[repeat(auto-fill,minmax(250px,1fr))] gap-5">
			{#each list as entry (entry.id)}
				<section animate:flip={{ duration: 150 }} class="flex flex-col justify-center border-b border-dashed border-b-weak pb-1">
					<span class="flex items-center gap-1">
						...
						<a href={entry.url} class="leading-normal text-primary font-semibold link truncate">{entry.data.title}</a>
					</span>
			...
			{:else}
				<div class="col-span-2 pt-[10vh] text-center text-secondary font-bold text-xl">No notes available</div>
			{/each}
		</header>
