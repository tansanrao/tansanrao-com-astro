...
{#if sensitive}
	<div transition:fade={{ duration: 150 }} class="flex flex-col items-center justify-end gap-6">
		...
		<div class="flex gap-3">
			<button class="min-h-11 font-bold text-background bg-red-500 py-1 px-2 rounded-md" onclick={() => (sensitive = false)}>
				I understand, continue reading
			</button>
			<a href={back} class="flex min-h-11 items-center font-bold text-background bg-secondary py-1 px-2 rounded-md">
				Return to list
			</a>
		</div>
	</div>
