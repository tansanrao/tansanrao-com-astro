...
let { entries, series: seriesList, tags: tagList }: { entries: any[]; series: string[]; tags: string[] } = $props();

/** Track initial load to parse URL parameters */
let initial = $state(true);

/** Pagination size */
const size: number = config.pagination?.blog || 15;
...
/** Filtered and paginated list of blog entries */
let list: any[] = $derived.by(() => {
	let filtered: any[] = entries
		.filter(entry => {
			// Check if entry matches the specified series
			let matchSeries = !series || entry.data.series === series;

			// Check if entry contains all specified tags
			let matchTags = tags.every(tag => entry.data.tags?.includes(tag));
	...
	<article class="flex flex-col gap-4 grow">
		{#each list as entry (entry.id)}
			<section animate:flip={{ duration: 150 }} class="flex flex-col">
				...
				<time datetime={entry.data.timestamp.toISOString()} class="font-mono text-[0.65rem] text-remark">{Time.toString(entry.data.timestamp)}</time>
			</section>
		{:else}
			<div class="pt-[10vh] text-center text-secondary font-bold text-xl">No blog posts available</div>
		{/each}
