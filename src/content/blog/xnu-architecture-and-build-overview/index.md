...
timestamp: 2026-07-30 14:50:00-04:00
series: XNU on Pi 
tags:
  - xnu
  - kernel
...
something different. After [my previous deep dive on Apple's Darwin OS and
XNU
...
porting it to non-Apple hardware!

XNU-on-Pi is my attempt to port XNU to a Raspberry Pi and getting as much of the
hardware working as possible over the next 3 weeks. For userland, I want to get
to a standard shell like bash or zsh and some basic utilities. Now that we have
LLMs to act as amazing semantic search engines over a codebase, I believe it'll
be significantly faster to debug and learn, allowing me to get pretty far in the
3 weeks I allowed myself. I'll be documenting the entire journey, my notes, and
my experience with including LLMs in a research/learning process through this
series. All of the source will also go up on Github as I make progress.

My plan so far is to do a bunch of reading on XNU's architecture, the way the
...
source, finding the `_start` method, and then tracing everything from there. Any
parts that I don't understand, I either search or let an LLM explain it to me.
The rest of this post is just a high-level summary of all my notes and I'll link
to the XNU source for the interesting bits. 
...
[Mach](https://en.wikipedia.org/wiki/Mach_(kernel)) is an operating system
kernel developed at Carnegie Mellon University by Richard Rashid and Avie
Tevanian to support operating system research. It is considered one of the
earliest examples of a microkernel. 
...
extensions, BSD device discovery, and `IOService` matching and lifecycle.
According to the
...
## SDK versus KDK
The SDK provides target headers and module maps, deployment-version information,
stubs and development tools, and a staging area for generated/private
dependencies under `/usr/local`. The KDK which is the Kernel Debug Kit, is tied
more closely to a particular macOS build and provides matching kernel symbols
and dSYMs, private kernel headers and metadata, per-machine kernel-support
archives, platform and monitor libraries that are used during XNU linking. For
apple silicon, KDK is required since a bunch of platform specific binaries are
distributed through the KDK and required when linking the kernel.

## Next Steps

With all of this background reading out of the way, I have a sense for where
everything is and how things are related to each other. I can now start hacking
with XNU. My next steps are to setup a proper environment to build in, setup
Qemu with the `raspi4b` model and correct DTBs, write a simple loader program
that the Pi can boot, and print Hello World to the UART port. Once we have a
loader program working, it'll be time to teach XNU about the BCM2711 SoC used by
the Pi, and see if we can get the loader to handoff cleanly to XNU boot and get
a hello world out of XNU.
